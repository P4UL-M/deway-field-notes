...
La production tourne sur un serveur personnel. Apache termine HTTPS, puis un routeur Nginx interne distribue les requêtes entre Astro et Payload. Umami reste isolé, avec sa propre base dans l'instance PostgreSQL partagée.
...
La stack Docker de production est décrite dans `compose.yaml`. Les secrets restent exclusivement dans le fichier `.env` du serveur. Une archive de l'ancien site statique est conservée hors du dossier déployé pour permettre un retour arrière manuel.

## Licence

Le code du projet est distribué sous licence MIT. Les polices embarquées conservent leurs licences SIL Open Font License respectives dans `astro-site/fonts/licenses/`.
